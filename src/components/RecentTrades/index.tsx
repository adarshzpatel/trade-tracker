...
import { Callout, Card, Col, Flex, Icon, Title } from "@tremor/react";
      ...
      <Col>
        {data?.map((it, index) => (
          <div key={`ro-${it.id}`} className="flex gap-4 justify-between group">
            ...
            <button
              onClick={() => deleteTrade(it.id)}
              className="hidden group-hover:block w-0 group-hover:w-10 -rotate-12 hover:scale-110 active:scale-100 hover:rotate-0 duration-300 transition-transform"
            >
